...
- **`matplotlib`** - единственная "тяжёлая" зависимость, нужна только для
  ...
  зависит.
- LLM/LangChain **не подключены** намеренно задание (без внешних API) выбран как основной, чтобы код гарантированно запускался локально без ключей. Место для LLM-варианта с fallback архитектурно предусмотрено.
...
1. Убрать служебные аннотации вида `(не 01.03.2025!)` - такие пометки
   ...
   не должны попадать в реальный OCR-текст.
