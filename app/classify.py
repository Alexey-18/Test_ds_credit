...
# Порог разрыва между первым и вторым местом (см. обоснование в RESULTS.md).
CONFIDENCE_GAP_THRESHOLD = 0.15

# Ключевые слова/фразы для каждого типа документа. Более длинные и
# специфичные фразы получают больший вес, т.к. реже дают ложные
# срабатывания.
_KEYWORDS: Dict[str, List[Tuple[str, float]]] = {
    "contract": [
        # "договор поставки" часто встречается и в счетах/актах как ссылка
        # на основание платежа ("Основание: Договор поставки № ... от ...") —
        # поэтому вес ниже, чем у фраз, характерных именно для тела
        # контракта.
